...
@if($data->status == 1)
<button class="btn btn-danger" id="informe-{{ $data->id }}" data-id="{{ $data->id }}" data-code={{ $data->order->code }}>Non</button>
@else
...
<script>
    $(document).ready(function() {
        $('#informe-{{ $data->id }}').click(function(e) {
            ...
            Swal.fire({
                title: 'Êtes-vous sûr?'
                , html: "<span style='font-size: 20px;'>Demande d'examen : <b>" + reportTestOrderCode + "</b> <br> Opération : <b>Patient informé => OUI </b></span><br><br> Voulez-vous vraiment effectuer cette opération ?"
                , icon: 'warning'
