...
<div class="row">
    <div class="col-12">
        <div class="page-title-box">
            ...
            {{-- <h4 class="page-title">Contrat : [{{ $detail_contrat->contrat->name }}]</h4> --}}
        </div>
    </div>
</div>
...
<div class="">
    ...
    <div class="card mb-md-0 mb-3 mt-4">
        <div class="card-body">
            ...
            <h5 class="card-title mb-0">Modifier un examen pris en compte</h5>

            <div id="cardCollpase1" class="collapse pt-3 show">
                <form action="{{ route('contrats.update_examen_reduction',$detail_contrat->id) }}" method="POST"
                    autocomplete="off">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="modal-body">
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-control select2" data-toggle="select2" id="example-select"
                                ...
                                <option value="{{ $test->id }}" {{ $test->id == $detail_contrat->test_id ? 'selected' :
                                    ''
                                    }}>{{ $test->name }} ({{
                                    $test->price }})</option>
                                @endforeach
                        ...
                        <div class="mb-3">
                            <label for="simpleinput" class="form-label">Réduction<span
                                    style="color:red;">*</span></label>
                            <input type="number" name="amount_remise"
                                value="{{ $detail_contrat->amount_remise ?? old('amount_remise') }}" id="amount_remise"
                        ...
                        <input type="hidden" name="contrat_id" value="{{ $detail_contrat->contrat_id }}"
                            class="form-control" required>
                    </div>
